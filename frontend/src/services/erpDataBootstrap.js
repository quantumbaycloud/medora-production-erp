...
export async function loadERPData() {
  ...
  const medicines = value(3, []);
  const suppliers = value(4, []);
  const purchases = value(5, []);
  const branches = value(6, []);
  const staff = value(7, []);
  const attendance = value(8, []);
  ...
  inventoryData.setRuntimeData({
    ...
    categories: [...new Set(medicines.map((m) => m.category).filter(Boolean))],
