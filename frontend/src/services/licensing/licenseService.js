import { api, getDeviceIdentifier } from "../../store/authService";

export const getDeviceId = getDeviceIdentifier;

const deviceName = () =>
  typeof navigator !== "undefined" ? navigator.userAgent.slice(0, 255) : "MEDORAX ERP";

const licenseService = {
  status(token, tenantId) {
    return api.get("/api/licensing/status", {
      params: { tenant_id: tenantId, device_id: getDeviceId() },
    });
  },
  ...
  bootstrap(token, tenantId, name = deviceName()) {
    return api.post(
      ...
      { device_id: getDeviceId(), device_name: name },
      { params: { tenant_id: tenantId } },
    );
  },
  ...
  activate(token, envelope, tenantId, name = deviceName()) {
    return api.post(
      ...
      { ...envelope, device_id: getDeviceId(), device_name: name },
      { params: { tenant_id: tenantId } },
    );
  },
